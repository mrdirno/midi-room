...
// Wish 44813890: "enable a toggle for lead in countdown". Player options: Skip the lead-in on new songs.
const LEAD_IN_KEY = 'midi-room.skip-lead-in.v1';
test('Skip the lead-in is off by default: nothing stored, and every frame is told false right after the background policy', async () => {
  const storage = storageDouble();
  const h = harness({ storage }); const session = await h.activate();
  assert.equal(h.nodes.get('skipLeadIn').attributes['aria-pressed'], 'false');
  assert.equal(h.nodes.get('skipLeadInState').textContent, 'Off');
  assert.equal(storage.map.size, 0, 'the default is not written down');
  const types = session.port.messages.map(message => message.type);
  assert.equal(types.indexOf('background-policy'), types.indexOf('midi-state') + 1, 'the background policy still follows the MIDI state');
  assert.equal(types.indexOf('lead-in-policy'), types.indexOf('background-policy') + 1);
  assert.equal(session.port.messages.find(message => message.type === 'lead-in-policy').skipLeadIn, false);
});
test('the switch on is stored, told to every open frame and to a frame opened later; off again tells them false', async () => {
  const storage = storageDouble();
  const h = harness({ storage }); const session = await h.activate();
  h.nodes.get('skipLeadIn').click(); await h.tick();
  assert.equal(storage.map.get(LEAD_IN_KEY), 'on');
  assert.equal(h.nodes.get('skipLeadIn').attributes['aria-pressed'], 'true'); assert.equal(h.nodes.get('skipLeadInState').textContent, 'On');
  assert.deepEqual(session.port.messages.filter(message => message.type === 'lead-in-policy').map(message => message.skipLeadIn), [false, true]);
  const second = await h.activate(new File(['<html>b</html>'], 'b.html'), true);
  assert.equal(second.port.messages.find(message => message.type === 'lead-in-policy').skipLeadIn, true);
  h.nodes.get('skipLeadIn').click(); await h.tick();
  assert.equal(storage.map.get(LEAD_IN_KEY), 'off');
  for (const item of [session, second]) assert.equal(item.port.messages.filter(message => message.type === 'lead-in-policy').at(-1).skipLeadIn, false);
  h.nodes.get('roomCheck').click();
  const journal = JSON.parse(h.nodes.get('roomLogText').value).journal.map(entry => entry.event);
  assert.deepEqual(journal.filter(event => event.startsWith('lead-in.')), ['lead-in.skip', 'lead-in.play']);
});
test('a remembered Skip the lead-in comes back on at the next visit', async () => {
  const storage = storageDouble({ [LEAD_IN_KEY]: 'on' });
  const h = harness({ storage });
  assert.equal(h.nodes.get('skipLeadIn').attributes['aria-pressed'], 'true'); assert.equal(h.nodes.get('skipLeadInState').textContent, 'On');
  const session = await h.activate();
  assert.equal(session.port.messages.find(message => message.type === 'lead-in-policy').skipLeadIn, true);
});
