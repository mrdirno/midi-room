...
export function preflightPlugin(plugin,runtimeSource,rate,timeout=5000,signal=null) {
  ...
  return new Promise((resolve,reject)=>{
    ...
    const worker=new Worker(moduleURL(code));
    let settled=false;
    ...
    function finish(error,result){if(settled)return;settled=true;clearTimeout(timer);signal?.removeEventListener('abort',abort);worker.terminate();error?reject(error):resolve(result);}
    ...
    worker.postMessage({build:plugin.engine.build,rate});
  });
}
  ...
  async initialize(){
    ...
    try{
      ...
      for(let i=0;i<this.rack.modules.length;i++){
        ...
        const module=await WebAssembly.compile(Uint8Array.from(atob(build.wasm),c=>c.charCodeAt(0)));
        if(this.disposed)throw Error('Rack initialization cancelled.');
        await context.audioWorklet.addModule(moduleURL(code));
        if(this.disposed)throw Error('Rack initialization cancelled.');
