...
function busScore(w){
 ...
 for(pi=0;pi<w.roster.length;pi++){
  ...
  for(i=0;i<evs.length;i++){
   ...
   out.push({at:at,off:at+dur,ch:ch,note:note|0,vel:Math.max(1,Math.min(127,Math.round(e.vel*126)+1))});
  }
 }
...
function busPublish(t){
 ...
 if(busWorld!==S.world){
  /* Keep the cursor. It is the high-water mark of what has already gone down the
     ...
     and a held +/- button rebuilds every 80 ms: measured 28 note-ons where 8 were
     owed over one second, arriving 4-11 ms off a 125 ms grid, while the engine
 ...
 /* The loop came round. Test it against the PREVIOUS playhead, never against
    ...
    first. */
 if(busLast>=0&&t<busLast-1e-3)busCursor=t-1e-3;
 busLast=t;
 var horizon=t+0.4,base=mr.now();
 for(var i=0;i<busList.length;i++){
  var n=busList[i];
  if(n.at<=busCursor)continue;
