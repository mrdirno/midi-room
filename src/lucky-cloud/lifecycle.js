...
function busPublish(t,seek){
 ...
 if(busFrom>=0){
  ...
  /* Forget what already went out only when the engine will play it again (a seek back), or when
     ...
     each note once, and the notes already sent inside the 400 ms horizon are among them. Forgetting
     those sent the next bar's downbeat twice whenever the progress bar was clicked just before its
...
function cloudInstall(){
 ...
 /* The room's 'Keep playing when the screen locks' switch (MidiRoom.keepPlaying) turns both
    stops off; standalone there is no MidiRoom and the page stops on hide and blur as before. */
 cloudOn(document,'visibilitychange',function(){if(document.hidden&&!(window.MidiRoom&&MidiRoom.keepPlaying))cloudStop();});cloudOn(window,'blur',function(){if(!(window.MidiRoom&&MidiRoom.keepPlaying))cloudStop();});
